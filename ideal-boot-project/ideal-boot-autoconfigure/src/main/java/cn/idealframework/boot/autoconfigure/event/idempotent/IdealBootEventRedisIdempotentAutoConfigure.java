...
import cn.idealframework.boot.autoconfigure.event.idempotent.properties.IdealBootEventRedisIdempotentProperties;
import cn.idealframework.boot.starter.module.event.EventModule;
import cn.idealframework.event.listener.IdempotentHandler;
import cn.idealframework.event.listener.impl.RedisIdempotentHandler;
...
public class IdealBootEventRedisIdempotentAutoConfigure {
  ...
  public IdempotentHandler idempotentHandler(@Nonnull StringRedisTemplate redisTemplate) {
    IdealBootEventRedisIdempotentProperties redis = properties.getIdempotent().getRedis();
    log.info("Use RedisIdempotentHandler");
    return new RedisIdempotentHandler(redis.getKeyPrefix(), redis.getKeyTimeout(), redisTemplate);
  }
}
