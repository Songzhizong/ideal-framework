...
import cn.idealframework.util.retry.predicate.ExceptionClassPredicate;
import cn.idealframework.util.retry.predicate.ExceptionPredicate;
...
public class RetryerBuilder<V> {
  ...
  /**
   * Configures the retryer to retry if an exception to the given class (or subclass of the given class) is
   * thrown by the call.
   *
  ...
  public Retryer<V> build() {
    ...
    return new Retryer<>(theAttemptTimeLimiter, theStopStrategy,
      theWaitStrategy, theBlockStrategy, rejectionPredicate, listeners);
  }

  /**
   * @param workerExecutor workerExecutor
   * @return the built AsyncRetryer
   */
  public AsyncRetryer<V> buildAsyncRetryer(@Nonnull ExecutorService workerExecutor) {
    AttemptTimeLimiter<V> theAttemptTimeLimiter =
      attemptTimeLimiter == null
        ? AttemptTimeLimiters.noTimeLimit()
        : attemptTimeLimiter;
