...
import cn.idealframework.trace.LogInfo;
 ...
 */
@CommonsLog
public class WebClientLogCollector implements LogCollector {
  /** 是否异步执行 */
  ...
  public WebClientLogCollector(boolean async, String url, WebClient webClient) {
    this(async, url, webClient, null, null);
  }

  public WebClientLogCollector(boolean async,
                               @Nonnull String url,
    ...
    if (retryMaxAttempts == null) {
      retryMaxAttempts = async ? 3 : 0;
    }
    if (retryFixedDelay == null) {
      retryFixedDelay = Duration.ofMillis(200);
    }
    this.async = async;
    this.url = url;
  ...
  @Override
  public void collect(@Nonnull LogInfo logInfo) {
    Mono<String> mono = webClient.post().uri(url)
      .contentType(MediaType.APPLICATION_JSON)
      .body(BodyInserters.fromValue(logInfo))
      .retrieve()
      .bodyToMono(String.class)
      .doOnNext(r -> log.debug("Collect logInfo result: " + r));
    Mono<String> retry;
    ...
    Mono<String> resume = retry.onErrorResume(throwable -> {
      ...
      log.info("Save logInfo ex: " + message);
